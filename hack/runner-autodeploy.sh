...
# Public runner label — R1 / R2 ONLY, derived from the rN- advertise-host. The hardware (which
# Mac) is deliberately never named, here or in any commit status, since these are public.
...
GH_TOKEN="$(cat "$REPO_ROOT/secrets/github-status.token" 2>/dev/null || printf '%s' "${GH_STATUS_TOKEN:-}")"
